#ifndef BNUMIO_H
#define BNUMIO_H

/************************************************************************************/
/*MAX_PATH_LENGTH - Максимальный размер пути к файлу							*/
/*DEFAULT_INPUT_PATH - Стандартное имя файла, из которого считывается число     */
/************************************************************************************/

#define MAX_PATH_LENGTH 151						
#define DEFAULT_INPUT_PATH "input.txt"
#define DEFAULT_OUTPUT_PATH "output.txt"


/************************************************************************************/
/*Функция для чтения числа из файла							*/
/*																					*/
/*int scanFile(IN char* path,OUT p_element numptr);									*/
/*		Входные параметры: char* path - путь к файлу, из которого			*/
/*										производить чтение				*/
/*		Выходные параметры: p_element numptr -									*/
/*		массив, в который будет помещено число, читаемое из файла				*/
/*																					*/	
/*		Возвращаемые значения: -1 - не удалось открыть файл						*/
/*								0 - число успешно считано из файла				*/
/*																					*/
/************************************************************************************/
int scanFile(IN char* path, OUT p_element numptr);


/************************************************************************************/
/*Функция для вывода числа в файл								*/
/*																					*/
/*int printFile(IN char* key, IN char* path,IN p_element numptr);		*/
/*		Входные параметры: char* key - ключ для открытия файла						*/
/*							w для перезаписи, a для добавления в конец					*/		
/*						char* path - путь к файлу									*/		
/*		 p_element numptr - указатель на число,							*/
/*								которое необходимо распечатать					*/
/*		Возвращаемые значения:  -2 - некорректный ключ для открытия файла    */
/*								-1 - не удалось открыть файл						*/
/*								 0 - число успешно распечатано в файл			*/
/*																					*/
/************************************************************************************/
int printFile(IN char* path,IN p_element numptr);



/************************************************************************************/
/*Функция для вывода числа в консоль в формате Little Endian				*/
/*																					*/
/*void printConsoleLE(IN p_element numptr);							*/
/*		Входные параметры: p_element numptr -									*/
/*				число, которое нужно вывести в консоль							*/
/*																					*/
/************************************************************************************/
void printConsoleLE(IN p_element numptr);


/************************************************************************************/
/*Функция для вывода числа в консоль в формате Big Endian					*/
/*																					*/
/*void printConsoleLE(IN p_element numptr);							*/
/*		Входные параметры: p_element numptr -									*/
/*				число, которое нужно вывести в консоль							*/
/*																					*/
/************************************************************************************/
void printConsoleBE(IN p_element numptr);

#endif BNUMIO_H